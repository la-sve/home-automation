...
/*  for (counter = 0, toRead = 0 ; counter < MAX_COUNTER && toRead != 3 ; counter++)
  ...
  while ((_i2cPort->requestFrom(HTU21D_ADDRESS, 3) != 3) && (counter < MAX_COUNTER)){
    delay(DELAY_INTERVAL);
    counter++;
  }
...
void HTU21D::reset(void) {
  ...
  _i2cPort->write(&cmd, 1);
  delay(15);
}

void HTU21D::triggerHumidity(void){
  _i2cPort->beginTransmission(HTU21D_ADDRESS);
  _i2cPort->write(TRIGGER_HUMD_MEASURE_NOHOLD);
  _i2cPort->endTransmission();
}
...
void HTU21D::triggerTemperature(void){
  _i2cPort->beginTransmission(HTU21D_ADDRESS);
